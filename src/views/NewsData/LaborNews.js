...
const LaborNews = () => {
  ...
  const deleteData = (id) => {
    ...
    const unsubscribe = db.collection('laborNews').doc(cardData).delete()
    return unsubscribe
  }

  useEffect(() => {
    const unsubscribe = db.collection('laborNews').onSnapshot((snapshot) =>
      setData(
